...
![](/images/githubpages/level10.png)
图7-1  LevelDb读取记录流程
...
##LevelDb日知录之八：Compaction操作
    ...
    先来看看minor Compaction的过程。Minor compaction 的目的是当内存中的memtable大小到了一定值时，将内容保存到磁盘文件中，图8.1是其机理示意图。 ![](/images/githubpages/level11.png)
...
如果选好了level L的文件A和level L+1层的文件进行合并，那么问题又来了，应该选择level L+1哪些文件进行合并？levelDb选择L+1层中和文件A在key range上有重叠的所有文件来和文件A进行合并。
 ...
 　　也就是说，选定了level L的文件A,之后在level L+1中找到了所有需要合并的文件B,C,D…..等等。剩下的问题就是具体是如何进行major 合并的？就是说给定了一系列文件，每个文件内部是key有序的，如何对这些文件进行合并，使得新生成的文件仍然Key有序，同时抛掉哪些不再有价值的KV 数据。
    ...
    图8.2说明了这一过程。
![](/images/githubpages/level12.png)
...
注：原文地址：http://www.samecity.com/blog/Index.asp?SortID=12
参考资料：1.维基百科：http://zh.wikipedia.org/wiki/LevelDB
